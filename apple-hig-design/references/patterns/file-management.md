...
**File provider app extension.** If your app can share its files with other apps, you can create a file provider app extension that displays a custom interface for importing, exporting, opening, and moving your app's documents. For developer guidance, see File Provider. An app extension is code you provide that people can install and use to extend the functionality of a specific area of the system; to learn more, see App extensions.
...
**Custom file management.** People have strong associations with the familiar file browsing experience of the Finder and most document-based apps. Use the default file browser unless you have an important reason to create a custom one.
...
**Finder Sync extensions.** If your app syncs local and remote files, you can create a Finder Sync app extension to express file synchronization status and control within the Finder. For developer guidance, see Finder Sync. For example, you can use a Finder Sync extension to:
...
**Key APIs**
- `DocumentGroupLaunchScene` — configures the iOS 18 / iPadOS 18 document launcher's title card, background, accessories, and file browser sheet

**Videos:** Build document-based apps in SwiftUI

## Web translation *(derived — not from Apple)*

Apple's HIG contains no web guidance. The mappings below apply the same principles to the web; they are inference, not Apple policy.

**"Help people be confident their work is preserved" → autosave to persistent storage, not just to memory.** Apple's rule against making explicit save an obligation maps to periodically writing to `localStorage`, IndexedDB, or a backend, on a timer and on `visibilitychange`/`beforeunload`. The web adds a failure mode Apple's platforms mostly hide from users: a closed tab can lose unsaved in-memory state instantly, so the autosave interval matters more on the web than on a platform where the OS itself manages app suspension gracefully.

**Native file system access is opt-in and far more constrained than Apple's Finder/Files integration.** The File System Access API (`showOpenFilePicker`, `showSaveFilePicker`) lets a web app read and write to a user-chosen location on disk, but it's Chromium-only as of this writing, requires a user gesture to invoke, and has no equivalent of a persistent "recent documents" list without extra permission bookkeeping. Where that API isn't available, the practical web substitute for "creating and opening files" is upload (`<input type="file">`) and download (an anchor with the `download` attribute) — a copy-based model, not the open-in-place model Apple describes.

**"Support people's understanding of the platform's file system" → there is no shared file system to lean on.** Apple's advice assumes a Finder or Files app every user already understands. A web app has no equivalent common ground — each app's file browser (if it has one) is bespoke, backed by whatever storage the app chose. The transferable principle is narrower: keep your own in-app file browser's structure predictable and consistent with itself, since you can't borrow platform familiarity.

**Quick Look previews → there's no system-wide preview generator, so each app renders its own.** Apple's Quick Look lets other apps preview a file your app produces without opening it. On the web, the nearest equivalent is rendering a thumbnail or preview server-side (or client-side, for supported formats) and serving that alongside the file — there's no OS-level hook other apps can call into, because there's no shared OS.

**Hiding file extensions → largely moot; the web already abstracts this away.** Because most web apps don't expose a raw filesystem, Apple's file-extension-visibility toggle doesn't have a direct analogue. Where you do let people download files, the filename (including extension) you set via the `download` attribute or `Content-Disposition` header is what they'll see in their OS file manager afterward — so the underlying discipline (choose a clear, correct extension) still applies, just one layer removed.

**Document launcher, file provider extensions, Finder Sync extensions → no web equivalent.** These are deep OS integrations (a full-screen app launcher, a file-provider that other apps browse into, a Finder extension that adds sync badges and menu items) that depend on being a first-class citizen of the platform's file system. A web app cannot register itself as a source other apps browse into, cannot add badges to a native file manager, and has no launcher surface comparable to `DocumentGroupLaunchScene`. Treat this guidance as platform-specific with no meaningful web translation.

## Do / Don't

| Do | Don't |
|---|---|
| Autosave periodically and on losing focus or closing | Require an explicit save action for normal editing |
| Provide an Add/New affordance alongside any keyboard shortcut | Rely on keyboard shortcuts as the only way to create a document |
| Show only contextually relevant file types in a custom browser or extension | List every file type regardless of what the current context accepts |
| Reflect the extension-visibility choice consistently across open/save UI | Show extensions in one dialog and hide them in another |
| Indicate unsaved changes clearly when autosave is off | Show an "unsaved changes" indicator when autosave is on |
| Use the platform's default file browser unless you have a strong reason not to | Build a custom file browser that behaves unlike the system's |
