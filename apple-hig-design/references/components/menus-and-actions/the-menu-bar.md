...
1. YourAppName (you supply a short version of your app's name for this menu's title)
2. File
3. Edit
4. Format
5. View
...
> **Note (Apple):** The View menu doesn't include items for navigating between or managing specific windows; the Window menu provides these commands.
...
> **Note (Apple):** The Window menu doesn't help people customize the appearance of windows or close them. To customize a window, people use commands in the View menu; to close a window, people choose Close in the File menu.
...
| Additional Item | — | Use a separator between your primary help documentation and additional items, which might include registration information or release notes. Keep the total number of items you list in the Help menu small to avoid overwhelming people with too many choices when they need help. Alternatively, consider linking to additional items from within your help documentation. |
...
> **Tip (Apple):** macOS automatically sets the width of a menu to hold the widest item, including dynamic menu items.
...
**Consider exposing app-specific functionality in other ways, too.** For example, you can provide a Dock menu that appears when people Control-click your app's Dock icon. People can hide or choose not to use your menu bar extra, but a Dock menu is always available when your app is running.
...
| App menu short name length | 16 characters or fewer recommended |

> **Source limitation:** The only numeric specifications the source states directly are the macOS menu bar height (24 pt) and the recommended 16-character limit for the app menu's short name. All other guidance on this page is structural (menu order, item order) rather than dimensional.
...
**Key APIs**
- `CommandMenu` — SwiftUI, defines a top-level menu bar menu
- `NSStatusBar` — AppKit, hosts menu bar extras
- `MenuBarExtra` — SwiftUI, creates a menu bar extra
- `isAlternate` — marks a dynamic (modifier-key-revealed) menu item
- `NSHelpManager` — wires up the Help menu's search field
...
**There is no direct web analogue to a persistent, OS-level menu bar.** A browser tab has no equivalent chrome that an individual web app can populate the way a Mac app populates the system menu bar — the browser owns that space entirely. The closest web pattern is an application-level menu bar widget rendered inside the page (common in web-based productivity apps that mimic desktop software), and it inherits none of the system's built-in behavior: no automatic Copy/Cut/Paste wiring, no automatic Full Keyboard Access support, no OS-managed width or truncation.

**"Support the default system-defined menus and their ordering" → don't reinvent conventions users already hold.** If you build an in-page menu bar, the transferable principle is to match the order and vocabulary web/desktop users already expect (File, Edit, View conventions carry over reasonably well from decades of desktop software), rather than inventing a novel taxonomy. This is a weak analogy, not a strong one — most web apps are better served by conventional top navigation or a command palette than by reconstructing a desktop menu bar.

**"Always show the same set of menu items, disable rather than hide" → applies directly to any custom menu UI.** This is a general menu/toolbar principle, not menu-bar-specific, and it transfers cleanly: keep disabled actions visible and grayed out so people can learn the full capability surface of the app, rather than making the available actions unpredictable.

**Keyboard shortcuts → respect the browser's and OS's reserved bindings.** Apple can guarantee Cmd-C means Copy everywhere. On the web you're a guest in the browser chrome; many of the "standard" bindings Apple describes (Cmd-W, Cmd-Q, Cmd-N) are owned by the browser and cannot be safely reassigned. Where you do implement in-page shortcuts, following the same *vocabulary* (which letter maps to which verb) is still worth doing, since it matches user expectation, even though the underlying mechanism differs completely.

**Dynamic (modifier-key) menu items have no clean web equivalent.** Detecting a held modifier key at the moment of a menu-open interaction is a native-UI pattern; recreating it in a web menu is possible via `keydown`/`keyup` state but is fragile across browsers and unfamiliar to web users, so it's rarely worth the friction it adds.

## Do / Don't

| Do | Don't |
|---|---|
| Support the standard system-defined menus and their ordering | Invent a custom top-level menu order |
| Always show the full set of menu items, disabling the unavailable ones | Hide menu items that are contextually unavailable |
| Use the same icons as the system for common actions | Design custom icons for Copy, Share, Delete, and similar standard actions |
| Support the standard keyboard shortcuts for standard menu items | Define custom shortcuts for actions that already have a standard one |
| Prefer short, one-word menu titles | Use long, multi-word menu titles that crowd the menu bar |
| Display the About item first, separated into its own group | Bury the About item among other app-menu items |
| Reflect your app's hierarchy in app-specific menu order | List app-specific menus in an arbitrary order |
| Provide a Window menu even with only one window | Omit Minimize/Zoom items, blocking Full Keyboard Access |
| Require only a single modifier key for a dynamic menu item | Require multiple simultaneous modifier keys to reveal a dynamic item |
| Offer custom Dock/menu bar extra functionality elsewhere too | Rely on a menu bar extra as the only way to reach a feature |
