...
Clear, accurate descriptions can help make a chart more approachable and accessible; to learn about additional ways to improve the accessibility of your chart, see Enhancing the accessibility of a chart below.
...
> *Image caption:* A weekly range chart and a monthly range chart from the Health app's Steps view, illustrating how a dynamic Y-axis upper bound adapts to the largest value in the current time period.
...
**Make every chart in your app accessible.** Charts — like all infographics — need to be fully accessible to everyone, regardless of how they perceive content. For example, it's essential to support VoiceOver, which describes onscreen content to help people get information and navigate without needing to see the screen. In addition to supplying accessibility labels that describe the components of your chart, you can enhance the VoiceOver experience by also using **Audio Graphs**. Audio Graphs provides chart information to VoiceOver, which constructs a set of tones that audibly represent a chart's data values and their trend; it also lets you present high-level text summaries that provide additional context.
...
**Make an interactive chart easy to navigate when using keyboard commands (including full keyboard access) or Switch Control.** By default, these input types tend to visit individual onscreen elements in a linear sequence, such as the sequence of values in a data file. If you want to provide a custom navigation experience in your chart, there are two main ways to do so. The first is to use accessibility APIs (such as `accessibilityRespondsToUserInteraction(_:)`) to specify a logical and predictable path through your chart's information — for example, letting people navigate along the X axis instead of jumping back and forth. The second — particularly useful if you need to present a very large dataset — is to let people move focus among subsets of values instead of navigating through all individual data points. Both of these customizations can also enhance the VoiceOver experience, even when your chart isn't interactive.
...
> *Image caption:* For the focused section of this cycling elevation chart, VoiceOver provides information about that portion of the route, including distance and elevation changes.
...
**Key APIs**
- `accessibilityRespondsToUserInteraction(_:)` — specify a logical, predictable navigation path through a chart's data for keyboard and Switch Control users
- `UIAccessibility.Notification` (UIKit) — announce chart changes to VoiceOver
- `NSAccessibility.Notification` (AppKit) — announce chart changes to VoiceOver

**Videos:** Bring Swift Charts to the third dimension · Design app experiences with charts · Design an effective chart

## Web translation *(derived — not from Apple)*

Apple's HIG contains no web guidance. The mappings below apply the same principles to the web; they are inference, not Apple policy.

**Marks, scales, and axes → this is exactly SVG/Canvas charting theory, and the vocabulary already matches.** Apple's anatomy section (marks, scales, plot area, axes, ticks, grid lines) is the same grammar used by web charting libraries built on the Grammar of Graphics (D3, Vega-Lite, Observable Plot). The concepts transfer directly; the web adds no new primitive here, so there's little to translate — just carry the vocabulary over.

**"Avoid relying solely on color" → this is the one place the web genuinely lags without deliberate work.** Apple's point-mark shape trick (two shapes for the two components of blood pressure) has an exact web equivalent: vary marker shape, dash pattern, or texture alongside color, and never encode a data dimension in hue alone. The web has no system-level guarantee here the way Apple's HIG can lean on platform-wide color-blind-safe defaults; a web chart's color choices are entirely the author's responsibility, so this rule matters more, not less, in a browser.

**"Make every chart accessible" / Audio Graphs → the ceiling is lower on the web, and that has to be said plainly.** Audio Graphs is a first-party VoiceOver feature: Swift Charts generates it for free, translating a chart's values and trend into audible tones plus a spoken summary. No equivalent shipping browser feature exists. The closest a web chart can get is: expose the underlying data in an accessible table alternative (often visually hidden but available to screen readers), give the chart container a clear ARIA role and label, and write the same kind of context-setting accessible description Apple asks for — chart type, what each axis represents, axis bounds — as static text or `aria-label`/`aria-describedby` content. This produces the text half of what Audio Graphs gives you; it does not produce the sonification half. Say so rather than implying the web has parity.

**"Hide visible text labels for axes/ticks from assistive technologies" → same instinct, `aria-hidden` on the redundant visual layer.** If the accessible description or data table already conveys axis names and values, marking the decorative SVG/canvas tick labels `aria-hidden="true"` avoids the screen reader announcing the same numbers twice, exactly as Apple's guidance intends.

**Fixed vs. dynamic axis range → this reasoning transfers with zero adaptation.** A meaningful fixed bound (battery 0–100%) versus a dynamic bound that tracks the data (Steps chart) is a data-modeling decision, not a platform one. The same judgment call applies verbatim in any web charting library.

**Responsive chart sizing → the web's real addition, and where Apple's guidance under-specifies for the medium.** Apple's "maximize the width of the plot area in a compact environment" assumes a small number of fixed size classes (compact, regular). The web has to handle a continuous range of container widths, so a web chart needs an explicit strategy Apple's guidance doesn't have to name: recompute tick density and label formatting at each breakpoint (don't just scale the SVG), collapse a multi-series legend into a tap-to-reveal control below a given width, and prefer abbreviating axis labels programmatically over letting them wrap or overlap. Apple's "keep vertical axis labels as short as possible" is the right instinct; a web chart has to apply it dynamically rather than choosing one label set at design time.

**Interaction and hit targets → the touch-target math is the same, pointer input is the added case.** Apple's advice to expand the hit target to the full plot area for small marks maps directly to web touch targets, with the same reduced-motor-control motivation. The web adds mouse hover as a first-class interaction Apple's touch/Digital Crown-oriented guidance doesn't cover in the same way; a hover tooltip is a reasonable web-native addition as long as the same information remains reachable by keyboard focus, not just by mouse position.

## Do / Don't

| Do | Don't |
|---|---|
| Choose a mark type (bar, line, point) based on what you want to communicate | Pick a chart style without regard to the comparison it needs to support |
| Combine mark types (e.g., points on a line) when it adds clarity | Layer marks without a clear communicative reason |
| Use a fixed axis range when min/max are meaningful for all data (e.g., battery 0-100%) | Force a fixed range onto data that varies widely |
| Use familiar tick sequences (0, 5, 10…) | Use unfamiliar intervals (1, 6, 11…) that require extra thought |
| Summarize a chart's main message in a title/subtitle | Make people read every mark to find the point |
| Supplement color with shape or pattern | Rely on color alone to distinguish data series |
| Provide accessibility labels with context (date, location) and real values | Use subjective terms like "rapidly" or "gradually" in accessibility labels |
| Expand hit targets to the full plot area for small marks | Leave marks too small to target with a finger or pointer |
| Align chart leading edges with surrounding interface elements | Let tick labels protrude past the chart's leading edge |
| On watchOS, favor at-a-glance information and simple interactions | Require complex chart interactions on watchOS |
