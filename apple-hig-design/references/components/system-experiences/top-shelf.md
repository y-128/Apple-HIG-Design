...
**If you don't provide the recommended full-screen content, supply at least one static image as a fallback.** The system displays a static image when your app is in the Dock and in focus and full-screen content is unavailable. tvOS flips and blurs the image, ensuring that it fits into a width of 1920 pixels at the 16:9 aspect ratio.

| Fallback static image | Size |
|---|---|
| Image size | 2320x720 pt (2320x720 px @1x, 4640x1440 px @2x) |
...
Not supported in iOS, iPadOS, macOS, visionOS, or watchOS. Top Shelf is exclusive to tvOS's Apple TV Home Screen.

## Specifications

See the image size tables under Dynamic layouts above — the fallback static image, the three sectioned content row aspect ratios (Poster, Square, 16:9), and the scrolling inset banner are the complete set of dimensions Apple publishes for this surface.

## Native implementation

**Related**
- Apple Design Resources (Top Shelf layout templates)

**Videos:** Mastering the Living Room With tvOS

## Web translation *(derived — not from Apple)*

Apple's HIG contains no web guidance, and Top Shelf has no web equivalent. It is a tvOS-only surface driven by Dock focus and a physical remote's Touch surface — a hover/focus-driven full-screen preview carousel that only exists because Apple TV navigation has no cursor and no scroll wheel, just directional focus movement. Nothing on the open web has that input model, so the layout templates, focus effects, and remote gestures don't map to anything.

Two narrow principles do generalize, independent of the surface: **feature dynamic, personalized content over static promotion** — the same reasoning that makes a web homepage hero rotate fresh content rather than a fixed banner — and **don't make a non-interactive image imply it's clickable**, which is just as true of a decorative hero image on the web as of a static Top Shelf fallback. The practice of shipping both `@1x` and `@2x` pixel assets for every declared point size is also a direct precedent for `srcset`/`image-set()` density-based image delivery on the web, even though the specific dimensions are tvOS-only.

## Do / Don't

| Do | Don't |
|---|---|
| Feature new, recommended, or personalized content | Show ads or prices in Top Shelf |
| Provide a dynamic, layered-image experience when possible | Rely on a static image when full-screen content is achievable |
| Supply at least one static fallback image at 2320x720 pt | Leave Top Shelf blank when full-screen content is unavailable |
| Provide a succinct title (and subtitle if needed) | Overload the title with information belonging in More Info |
| Provide 3–8 images for a scrolling inset banner | Use more than 8 banner images or fewer than 3 |
| Bake required text into the image, with an accessibility label | Rely on labels under content in a scrolling inset banner |
| Load enough images to span the full row width | Ship a sectioned content row that doesn't fill the screen |
