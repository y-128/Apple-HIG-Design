...
> **Note (Apple):** The following guidance applies to apps that run in iOS and iPadOS. To learn about using ARKit to create immersive augmented reality experiences in visionOS, see ARKit.
...
> *Image caption:* App-specific indicator.
...
> *Image caption:* Direct manipulation, compared with indirect controls.
...
> *Image caption:* Limit movement to the two-dimensional surface on which the object rests. Limit object rotation to a single axis.
...
> **Note (Apple):** Regardless of the purpose of your app, don't use scaling as a way to adjust the distance of an object. If you enlarge a distant object in an effort to make it appear closer, the result is a larger object that still looks far away.
...
> *Image caption:* Prefer a 3D hint in a 3D context. If necessary, use a 2D hint in a 3D context.
...
> *Image caption:* Camera view, and detail view.
...
> *Image caption:* Sufficient lighting, compared with insufficient lighting.
...
> *Image caption:* AR badge, and glyph-only AR badge.
...
> **Source limitation:** The rest of this page's guidance is explicitly scoped by Apple to iOS and iPadOS (see the note under Core guidance above). Apple directs visionOS developers to the separate ARKit developer documentation rather than covering visionOS-specific design guidance on this page itself, so this reference does not — and cannot — extend the iOS/iPadOS interaction, coaching, and gesture guidance above to visionOS as verified Apple guidance.

## Specifications

| Property | Value |
|---|---|
| Scene update rate | 60 times per second, to avoid objects appearing to jump or flicker |
| Delay before removing objects attached to a lost reference image | Up to 1 second |
| Reference images in use at once (recommended maximum) | 100 or fewer distinct images |
| Face-tracking loss indicator threshold | About half a second (0.5s) of lost tracking before showing a visual indicator |
| AR glyph minimum clear space | 10% of the glyph's height |
| AR badge minimum clear space | 10% of the badge's height |
...
**Key APIs**
- ARKit — the framework underlying all guidance on this page
- `ARCoachingOverlayView` — the system-provided coaching UI for initialization and relocalization
- `ARTrackedRaycast` — used to refine an object's position as surface detection improves
- `isCollaborationEnabled` — enables multiuser AR experiences with automatic map merging

**Videos:** Qualities of great AR experiences · Explore ARKit 5

## Web translation *(derived — not from Apple)*

Apple's HIG contains no web guidance, and augmented reality is the most platform-bound topic in this set. Most of this page depends on ARKit-specific hardware capabilities — depth sensing, plane detection, image/object recognition, multiuser map merging, occlusion against a live camera feed, relocalization after an interruption — that a web page cannot access the way a native app can. Say this plainly rather than force a mapping.

**WebXR is the only partial analogue, and it's a genuinely partial one.** The WebXR Device API (`navigator.xr`) offers browser-based access to camera-fed AR sessions, hit-testing for surface detection, and anchors, covering some of the same ground as ARKit's plane detection and object placement. But Safari — the only browser on Apple's own platforms — has historically not shipped WebXR's immersive-AR module, which means a WebXR-based AR feature built for the general web will not reach iPhone or iPad users through Safari the way a native ARKit app does. Treat WebXR as viable for AR on the open web broadly, but verify current Safari support before betting an Apple-platform-facing feature on it — as of this writing it remains the weak link.

**AR Quick Look is the one genuine, first-party bridge from a web page into Apple's native AR stack, and it's worth using instead of trying to rebuild AR in the browser.** A web page can link to a USDZ or Reality file with `rel="ar"`; tapping it on iOS or iPadOS hands off from Safari into the system AR Quick Look viewer, which gets ARKit's real plane detection, lighting, and object placement — the same quality bar this page describes — without the site needing to implement any of it. For a website that wants to offer "view this in AR" on an Apple device specifically, this is the practical answer, not WebXR.

**The principles that transfer are the ones about communicating with people, not the ones about the 3D interaction model itself.** A few translate cleanly to any complex or unfamiliar feature on the web, immersive or not: use approachable, jargon-free language instead of technical terms (Apple's "Unable to find a surface. Try moving to the side" versus "Unable to find a plane. Adjust tracking." is a general lesson about avoiding internal terminology in user-facing copy); give specific, friendly suggestions when something fails rather than a generic error; and let people reset a failed or confusing state rather than making them wait it out. None of these require AR to be true.

**The "View in AR" badge is a UI convention that already exists on the web, independent of ARKit.** E-commerce product pages commonly mark items as viewable in AR (via a Quick Look link or a WebXR fallback) with a small badge or icon on the product photo — the same "use it consistently, keep it out of the way of the image, don't apply it to items that don't actually support it" guidance Apple gives for its own AR badge applies to that convention regardless of which underlying technology powers the AR view.

**Everything else on this page — direct manipulation gestures on 3D objects, occlusion against a live camera feed, multiuser map merging, image/object recognition, relocalization after backgrounding — assumes a camera-fed, depth-aware runtime that the ordinary web platform does not provide.** Where WebXR does cover this ground, its interaction model (controller/hand input, its own anchor and hit-test APIs) is different enough from ARKit's that porting this page's specific interaction guidance isn't reliable; treat WebXR AR as its own design surface with its own conventions, not as "ARKit guidance restated in JavaScript."

## Do / Don't

| Do | Don't |
|---|---|
| Devote as much of the screen as possible to the AR experience | Clutter the screen with controls that diminish immersion |
| Use friendly, conversational language for instructional text | Use technical terms like ARKit, world detection, or tracking in user-facing copy |
| Respond instantly when people place an object, then subtly refine it | Wait for perfectly accurate surface data before placing an object |
| Prefer direct manipulation of 3D objects when people are stationary | Force indirect 2D controls when direct touch would be more intuitive |
| Use the system coaching view for initialization and relocalization | Leave people without guidance during AR initialization |
| Let people reset the experience when it doesn't meet expectations | Force people to wait indefinitely for conditions to improve |
| Introduce motion gradually and consider people's comfort and safety | Encourage large, sudden, or sweeping movements right away |
| Use the AR glyph and badges exactly as provided, unaltered | Alter glyph/badge colors, or apply them to non-ARKit experiences |
| Maintain 10% clear space around the AR glyph and badge | Let other elements crowd or occlude the glyph or badge |
