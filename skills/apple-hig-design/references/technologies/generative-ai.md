...
**Factor processing time into your design.** Latency is how much time it takes for a model to produce an output. Non-generative models, such as body position tracking in ARKit and the Vision machine learning framework, typically have low latency and are suitable to run in real-time. Generative models typically take longer to produce a result, so design a loading experience or generate in the background while a person uses another part of the app. For guidance, see Loading.
...
**Key APIs**
- Foundation Models framework — requires a compatible device with Apple Intelligence turned on
- ARKit — cited as a low-latency, non-generative example (body position tracking)
- Vision framework — cited as a low-latency, non-generative example

**Videos:** What's new in the Foundation Models framework · Explore prompt design & safety for on-device foundation models · Create UI prototypes using agents in Xcode

## Web translation *(derived — not from Apple)*

Apple's HIG contains no web guidance. The mappings below apply the same principles to the web; they are inference, not Apple policy. Like Machine learning, this page transfers unusually well: it's about the ethics and mechanics of putting a probabilistic, sometimes-wrong system in front of a person, and the web now runs the same category of feature — chat assistants, AI writing tools, image generation, summarization — under the same open questions of disclosure, control, and failure.

**"Design your experience responsibly" → treat non-determinism as a design constraint, not a bug to hide.** Apple's observation that the same input can produce different outputs each time is exactly as true of a web LLM call as of an on-device model. The web-native implication is to design the UI around variance from the start — stable layout that doesn't jump when output length differs, visible regeneration rather than an assumption of one correct answer — instead of treating each odd output as an edge case to patch later.

**"Keep people in control" → Edit, Undo, Retry, Adjust are the actual accessibility layer for AI UIs.** Apple's list of controls near generated content is the single most portable idea on this page: nearly any web AI feature — a rewritten paragraph, a generated image, a summarized thread — needs a visible, low-friction way to dismiss, retry, or revert, placed next to the content itself rather than buried in a menu. A "regenerate" button and an "undo" affordance are cheap to build and are what most web AI UIs are missing.

**"Clearly identify when and where you use AI" → disclosure is a harder requirement on the web than Apple's framing suggests.** Apple asks developers to never let someone believe they're interacting with a human when they're not. On the web this rule is trending toward a legal one, not just an ethical one, in several jurisdictions requiring disclosure of AI-generated content, AI chat agents, and synthetic media. Treat Apple's transparency guidance as a floor, and check current regulation for the regions the site serves — the web audience is typically broader and more jurisdictionally mixed than a single app's install base.

**Privacy: on-device vs. server-based → the client/server boundary already exists on the web, but the trust story is weaker.** Apple's on-device-vs-server framing maps to running inference in-browser (via WebGPU or a small local model) versus calling a server API. The web version of "be transparent about what's shared" needs to work harder: a native app's privacy nutrition label and OS-level permission prompts don't exist for a random website, so the disclosure has to live in the product's own UI — what's sent to the model provider, whether it's used for training, and what a user can opt out of — stated plainly, not buried in a privacy policy.

**Hallucinations → the guidance to scope requests narrowly transfers directly, and the "avoid factual claims you can't verify" rule matters more on the open web.** Apple's advice — don't ask a model to produce verified facts unless you know it has access to accurate, current information, and warn people that output may be wrong — is arguably more urgent for web products, where retrieval-augmented answers sit next to search results and get treated with search-engine-level trust by default. A visible "AI responses may contain mistakes" note, and grounding claims in cited sources where possible, does the work Apple's guidance describes.

**Irreversible actions → confirm before an agent takes action on someone's behalf.** This maps cleanly onto the emerging class of web AI agents that can click through checkout flows, send emails, or modify account settings. Apple's rule — get confirmation before anything destructive or hard to undo, and don't automate it away by default — is close to a hard requirement for any web agent with write access to a user's data or money.

**Latency and "reassuring feedback during generation" → this is where the web has the *better* native toolset.** Streaming text output, skeleton loaders, and progress messaging are mature, well-supported web patterns; Apple's advice to replace "Processing…" with a specific description of what's happening ("Summarizing key themes from your notes") applies unchanged and is easy to implement with a streamed response updating incrementally rather than a blocking spinner.

**Feedback on outputs → thumbs up/down plus a text channel, kept voluntary and out of the way.** Apple's guidance to make feedback optional, low-friction, and positioned so it doesn't interrupt the experience is a near-literal description of good practice for AI chat and generation UIs on the web. The one gap Apple's guidance doesn't address for the web: anonymous or logged-out users generate a much higher volume of low-signal feedback, so plan for how feedback is weighted or rate-limited before treating it as ground truth.

## Do / Don't

| Do | Don't |
|---|---|
| Design around output variance from the start | Assume one input always produces the same output |
| Put Edit, Undo, Retry, and Adjust near generated content | Bury correction controls in a settings menu |
| Clearly disclose when content or an interaction is AI-generated | Let someone believe they're talking to or reading from a human |
| Offer a non-AI fallback when it's reasonable to | Make an AI feature the only path when it isn't essential |
| Scope requests narrowly and warn that output may be wrong | Ask a model for verified facts without confirming it has accurate data |
| Ask for confirmation before irreversible or significant actions | Automate destructive or hard-to-undo actions by default |
| Give specific, descriptive progress messages during generation | Show a generic "Processing…" spinner with no context |
| Make feedback voluntary and easy to give | Interrupt the experience to request feedback |
| Test with adversarial, vague, and sensitive prompts before shipping | Ship without testing out-of-scope or poorly phrased requests |
