...
**Adhere to image requirements.** Use a 1800×1200 px PNG or JPEG image without transparency.
...
> *Image caption:* App Clip Code with the App Clip logo, and App Clip Code without the App Clip logo.

App Clip Codes always use the designs Apple provides and follow size, placement, and printing guidelines. Choose between the badge design that uses the App Clip logo or, when space is at a premium, a design without it. Create App Clip Codes that use a default color pair, or choose custom foreground and background colors. For developer guidance, see Creating App Clip Codes.
...
**Don't create App Clip Codes that are too small.** App Clip Codes must adhere to the minimum size specifications — see Specifications below.
...
| Subtitle | Maximum 56 characters |
...
### Distance-to-code ratio

- Maximum ratio: 20:1 (distance to code diameter)
- Recommended ratio for reliable scanning: 10:1
- Worked example: a code scanned from 40 inches (101 cm) away needs to be at least 4 inches (10.16 cm) in diameter

### Cylindrical surface placement

The App Clip Code's width must not exceed one-sixth of the cylinder's circumference.
...
| Property | Value |
|---|---|
| Raster resolution (from SVG) | At least 600 ppi |
| Minimum print resolution | 300 dpi |
| Color conversion intent | Relative colorimetric (media-relative) |
| CMYK profile | "Generic CMYK ICC profile" (CMYK printers) or "Gracol 2013 ICC profile" (CMYKOV printers) |
| Color tolerance | CIELab Delta E of 2.5 |
| NFC tag type | Type 5 |
| NFC tag minimum diameter | 35 mm (or equivalent size) |
...
**Key APIs**
- `SKOverlay` — displays the in-App-Clip prompt to download the full app at a natural pause point
- App Clip Code Generator — command-line tool for generating App Clip Codes (alternative to App Store Connect)
- Sign in with Apple — recommended for account creation without on-device data retention
- Apple Pay — recommended for express checkout without typed payment entry
- NFC Tag Reader (Control Center) — used to scan NFC-integrated App Clip Codes

**Videos:** What's new in App Clips · Build light and fast App Clips

## Web translation *(derived — not from Apple)*

Apple's HIG contains no web guidance, and App Clips are a genuinely platform-bound feature: App Clip Codes, NFC-integrated tags, App Store Connect provisioning, `SKOverlay`, and the OS-level discovery surfaces (Siri Suggestions, Maps, the system app banner) have no open equivalent a website can plug into. Say this plainly rather than force a mapping — there is no "App Clip Code" for the web, and no system-level mechanism that lets a browser cede that much discovery and trust to a third-party site the way iOS does for a verified App Clip.

The one principle that transfers cleanly, and is worth stating on its own terms rather than as a strained analogy, is the **underlying design discipline**: a task-focused, installation-free, near-instant experience reachable from a physical or shared trigger. A restaurant's QR code that opens a mobile-optimized ordering page instead of forcing an app download, or a "no-install" web checkout linked from a poster, is doing the same job App Clips do — just through general-purpose web mechanisms (QR code → URL → responsive page) instead of a purpose-built OS feature. When building that kind of lightweight web flow, several of Apple's App Clip rules translate directly because they're really about minimizing friction under a "person is standing here right now" constraint, not about App Clips specifically:

**"Ensure people can use it immediately" and "keep it small"** map onto page weight and time-to-interactive for a QR-triggered web page. A person scanning a code at a bike rack or a table has the same low tolerance for a slow load that Apple describes for App Clip launch — treat the target page like a performance-critical landing page, not a general site page.

**"Avoid requiring an account before providing value"** and **"make it easy to pay"** are standard, provider-agnostic e-commerce and checkout guidance that predates App Clips and applies with the same force on the web: ask for the minimum information needed to complete the task, and support fast checkout methods (Apple Pay on the web, saved payment methods, autofill) rather than a full account-creation flow.

**"Design a linear, focused UI without tab bars or complex navigation"** is the same instinct behind minimal, single-purpose landing pages generally — a QR-triggered web page should behave like a task, not like a site with a nav bar into unrelated content.

What the web cannot replicate is the trust and provenance layer: an App Clip Code's recognizable badge design and Apple's code-signing give people confidence the flow is legitimate before they scan, and Apple's Legal requirements section exists precisely because that trust is a protected asset. A QR code pointing at an arbitrary URL carries none of that built-in guarantee — a person has no way to know where it leads until the page loads, which is a real, unresolved gap between the two approaches, not a detail to gloss over.

## Do / Don't

| Do | Don't |
|---|---|
| Let people complete the task or demo without installing the full app | Require a full app install to finish what the App Clip started |
| Limit features to what's necessary for the task at hand | Reserve advanced features for the App Clip instead of the app |
| Use native components for an app-quality experience | Use web views in an App Clip |
| Show the most relevant screen immediately on launch | Show a splash screen or make people wait |
| Offer Apple Pay and Sign in with Apple to reduce friction | Require account creation before the App Clip provides value |
| Use the Apple-generated App Clip Code design unmodified | Apply filters, shadows, gradients, or custom shapes to a generated code |
| Keep the App Clip Code flat, unobstructed, and upright | Overlay text/logos on the code, animate it, dim it, or rotate it |
| Recommend the full app once, at a natural pause | Interrupt tasks or repeatedly prompt for a full app install |
| Send notifications only in response to an explicit user action | Send purely promotional push notifications from an App Clip |
| Test printed codes at a variety of angles before distribution | Distribute printed codes without verifying scannability |
