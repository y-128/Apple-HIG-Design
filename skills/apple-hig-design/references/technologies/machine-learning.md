...
Features powered by machine learning use models to perform tasks that make people's experiences more unique, like curated feeds, customized trip itineraries, and personalized search suggestions. These features depend on well-designed models as much as they depend on a well-designed UI and user experience. For insights into the process of designing models, see Create ML.
...
No additional considerations for iOS, iPadOS, macOS, tvOS, visionOS, or watchOS. The guidance in this page applies uniformly across all six platforms.
...
**Videos:** Explore prompt design & safety for on-device foundation models · Discover machine learning & AI frameworks on Apple platforms

## Web translation *(derived — not from Apple)*

Apple's HIG contains no web guidance. The mappings below apply the same principles to the web; they are inference, not Apple policy. This page is unusually transferable: almost none of it depends on iOS-specific APIs, and most of it is about the relationship between a person and a system that produces uncertain, personalized output — a relationship the web has just as much as native apps do, in recommendation feeds, search ranking, autocomplete, spam filters, and now LLM-backed features.

**Critical vs. complementary → decide before you build whether a ML/AI feature can fail gracefully.** Apple's distinction maps directly: if your web app's core function still works with the model turned off, treat mistakes as low-stakes and don't gate the UI on the model responding. If a feature *is* the product — an AI writing assistant, a recommendation engine that is the whole feed — a web user has exactly the same reduced tolerance for bad output that Apple describes, and a slow or wrong response reads as the product failing, not a feature failing. Design a visible degraded state for both cases; don't let a stalled model call become a silently broken page.

**Explicit vs. implicit feedback → thumbs up/down vs. clickstream, translated faithfully.** The web has robust telemetry for implicit feedback (dwell time, scroll depth, clicks, abandoned carts) and the same low-conversion problem Apple describes for explicit feedback (a thumbs-down button gets used by a tiny fraction of users). Apple's rule to make explicit feedback voluntary and to word it in concrete consequences rather than vague terms like "dislike" transfers unchanged — a web feedback control that says "Show fewer results like this" is more useful than a bare down-vote icon, on any surface. The rule to act immediately and persist the change also transfers directly: if a user hides a recommendation and it reappears after a page reload because the preference wasn't written to a durable store, the trust cost is the same one Apple describes.

**Calibration → onboarding flows and permission requests.** Apple's calibration guidance — explain why before asking, collect the minimum, make progress visible, allow cancellation without judgment, let people revisit the data later — maps onto any web onboarding step that front-loads data collection (a taste quiz before a recommendation engine works, a permissions prompt before geolocation-based features activate). The web-specific risk Apple's guidance doesn't have to deal with is abandonment: a multi-step calibration flow on the web loses people at every additional screen in a way a captive native app does not, so the "make it quick" rule carries more weight here, not less.

**Mistakes, corrections, and confidence → the core of responsible AI-feature design on the web, and where the mapping runs deepest.** Apple's three linked ideas — that mistakes are inevitable and must be anticipated, that people need an easy correction path rather than being made to feel at fault, and that confidence should be translated into something actionable rather than shown as a raw percentage — describe almost exactly the emerging best practice for LLM-backed web features. A chat interface that lets someone edit and resend a prompt is a guided correction in Apple's sense; a "regenerate" button is a lower-effort variant of the same idea. Apple's warning against displaying a bare confidence number ("97% match" tells people nothing actionable) applies with more force to generated text and images, where a numeric confidence score is even less legible to a general audience than a search-relevance score is. The actionable-framing alternative Apple recommends — "This is a good time to buy" instead of a percentage — is the same instinct behind good AI UX copy that states what the system is uncertain about rather than how uncertain it is.

**Attribution → disclosure and explainability, which the web needs more of, not less.** Apple's rule that an attribution should explain the basis for a result without exposing the mechanism, stay factual, and avoid implying the system understands someone's emotions, maps onto "why am I seeing this" explanations in web recommendation UIs and onto AI-generated-content disclosure. The web context adds an obligation Apple's guidance doesn't carry the same way: growing regulatory and platform expectation to disclose when content is AI-generated at all, not just why it was suggested. Treat Apple's attribution guidance as the *tone* to use once you've already decided to disclose, not as a substitute for disclosure itself.

**Limitations → setting expectations before the feature runs out of runway.** Apple's advice to state limitations up front and explain in-context when a result is degraded because of them is directly portable — a "this may not always be accurate" note near a generated summary, or an explanation that a recommendation engine is cold-starting for a new account, does the same expectation-setting work on the web that Apple describes for Memoji's low-light warning. Where the web genuinely differs is scale and anonymity: Apple's examples assume a single person with an ongoing relationship to one app, while a lot of web ML/AI-driven functionality serves anonymous or first-time visitors with no interaction history to calibrate against, so the "explain the limitation in context" instinct has to do more work than "remember what happened last time."

## Do / Don't

| Do | Don't |
|---|---|
| Request explicit feedback only when necessary, and make it voluntary | Make providing feedback feel mandatory |
| Describe feedback options in concrete, consequence-specific language | Use vague terms like "dislike" |
| Act immediately on explicit feedback and persist the change | Let corrected or hidden content reappear later |
| Secure and disclose how implicit feedback is gathered and shared | Let people discover cross-app data sharing by surprise |
| Keep calibration minimal, quick, and cancelable without judgment | Ask people to calibrate more than once without reason |
| Give people an easy, familiar way to correct mistakes | Rely on corrections to compensate for low-quality results |
| Translate confidence into actionable language or ranking | Display a raw confidence percentage and expect it to be meaningful |
| Keep attributions factual and specific enough to be useful | Write attributions that imply understanding of someone's emotions |
| State limitations before or in the context of a poor result | Let a feature seem to work "intermittently" with no explanation |
