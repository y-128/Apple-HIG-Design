...
**Key APIs**
- Swift Charts — the framework for building the chart components this guidance describes
...
**"Not every collection of data needs a chart" → resist the reflex to visualize by default.** The same judgment call applies on the web: a sortable, searchable table often serves a user better than a chart component that adds a rendering library, a color-decoding step, and an accessibility burden for no interpretive gain. Choose a chart only when the visual form itself carries meaning — trend, proportion, distribution — that a table would obscure.

**"Make every chart accessible" → this is harder on the web than the HIG implies, and worth naming plainly.** Apple's platforms give charts a first-class accessibility path: an SF Symbols-adjacent, VoiceOver-integrated element that exposes both a description and interactive per-datapoint access. The web has no equivalent single mechanism. An SVG or canvas chart is, by default, a blank shape to a screen reader. Reaching parity with Apple's expectation means deliberately building three things Apple gets closer to free: a text alternative (a caption or summary sentence, which doubles as the "descriptive headline" Apple also recommends), a programmatically associated data table as a fallback, and keyboard-navigable focus stops on individual data points if the chart is interactive at all. Treat "accessible chart" as a construction requirement, not a checkbox.

**"Keep a chart simple, reveal detail gradually" → progressive disclosure via interaction, not via zoom-and-hope.** The web temptation is to cram a dense chart into a small card and let people pinch-zoom. Apple's actual guidance is closer to tiered UI: a glanceable small version, and a separate, more detailed view reachable on demand. That pattern translates directly — a summary chart that opens to a full interactive one on click or tap, rather than one chart trying to serve both roles at every viewport size.

**"Consistency across charts" → shared color and scale encoding, enforced by a token system.** Apple's advice to keep chart type, color, and annotation consistent across a dataset maps cleanly to using the same categorical and sequential color tokens across every chart component in a design system, rather than letting each chart instance or each engineer pick colors ad hoc. Where the web genuinely differs: Apple's charts render natively with the OS's color management and Dynamic Type; a web chart library often bakes in fixed pixel font sizes and RGB values that need explicit work to respect user font-size and `prefers-color-scheme` preferences the way the rest of a well-built page does.

## Do / Don't

| Do | Don't |
|---|---|
| Use a chart when the data's visual form itself communicates something | Chart data that would be equally or more useful as a scrollable, sortable list |
| Offer a simple glanceable version with detail available on demand | Pack maximum data density into a single chart view |
| Provide accessibility labels and interactive accessibility elements for every chart | Rely on the visual chart alone to convey the information |
| Add descriptive titles, subtitles, or annotations to highlight takeaways | Treat a headline or summary as a substitute for accessibility labels |
| Keep chart type, color, and style consistent across charts sharing a dataset | Use a different chart type for each view of the same dataset without reason |
| Size a chart to fit its functionality and level of detail | Undersize a chart people need to interact with, or oversize a glanceable one |
