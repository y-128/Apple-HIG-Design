...
> **Developer note (Apple):** Prefer using WidgetKit to develop complications for watchOS 9 and later. For guidance, see Migrating ClockKit complications to WidgetKit. To support earlier versions of watchOS, continue to implement the ClockKit complication data source protocol (see `CLKComplicationDataSource`).
...
**Provide a set of static placeholder images for each complication you support.** The system uses placeholder images when there's no other content to display for your complication's data. For example, when people first install your app, the system can display a static placeholder while it checks to see if your app can generate a localized placeholder to use instead. Placeholder images can also appear in the carousel from which people select complications. Complication image sizes vary per layout (and per legacy template), and the size of a placeholder image may not match the size of the actual image you supply for that complication.
...
Available regular-size layouts: Closed gauge image, Closed gauge text, Open gauge image, Open gauge text, Open gauge range, Image, Stack image, Stack text.
...
> **Note (Apple):** The system applies a circular mask to each image.
...
If you want to design an oversized treatment of important information that can appear on the X-Large watch face — for example, the Contacts complication, which features a contact photo — use the extra-large versions of the circular family's layouts. These layouts let you display full-color images, text, and gauges in a large circular region that fills most of the X-Large watch face. Some of the text fields can support multicolor text.

Available extra-large layouts: Closed gauge image, Closed gauge text, Open gauge image, Open gauge text, Open gauge range, Image, Stack image, Stack text.
...
> **Note (Apple):** The system applies a circular mask to the circular, open-gauge, and closed-gauge images.
...
Available layouts: Circular image, Gauge image, Gauge text, Stack text, Text image.
...
**Utilitarian small** layouts are intended to occupy a rectangular area in the corner of a watch face, such as the Chronograph and Simple watch faces. The content can include an image, interface icon, or a circular graph. Available layouts: Flat, Ring image, Ring text, Square.
...
> **Source limitation:** The PDF text extraction garbled the Flat row's 41mm value in this table (it rendered as "10.5-23.5x21 pt (21-47x21 @2x)"). The value above is taken from the identical Flat-layout row that appears cleanly under Utilitarian large, later in this section — both utilitarian layouts specify the same Flat text metrics, and that occurrence extracted without corruption.

**Utilitarian large** layout is primarily text-based, but also supports an interface icon placed on the leading side of the text. This layout spans the bottom of a watch face, like the Utility or Motion watch faces. Available layout: Large flat.
...
Available layouts: Standard body, Text gauge, Large image.
...
> **Note (Apple):** Both large-image layouts automatically include a four-point corner radius.
...
Circular small templates display a small image or a few characters of text. They appear in the corner of the watch face (for example, in the Color watch face). Available layouts: Ring image, Ring text, Simple image, Simple text, Stack image, Stack text.
...
> **Note (Apple):** In each stack measurement, the width value represents the maximum size.

#### Modular small

Modular small templates display two stacked rows consisting of an icon and content, a circular graph, or a single larger item (for example, the bottom row of complications on the Modular watch face). Available layouts: Columns text, Ring image, Ring text, Simple image, Simple text, Stack image, Stack text.
...
Modular large templates offer a large canvas for displaying up to three rows of content (for example, in the center of the Modular watch face). Available layouts: Columns, Standard body, Table, Tall body.
...
Extra large templates display larger text and images (for example, on the X-Large watch faces). Available layouts: Ring image, Ring text, Simple image, Simple text, Stack image, Stack text.
...
Complications are exclusive to watchOS. Not supported in iOS, iPadOS, macOS, tvOS, or visionOS.
...
**Key APIs**
- `CLKComplicationDataSource` — the ClockKit protocol for complications targeting watchOS versions before 9
- `WidgetRenderingMode` — detects the current rendering mode, including tinted mode
- `placeholder(in:)` — supplies placeholder content when there's no data to display
- `WidgetFamily.accessoryRectangular` — declares support for a rectangular complication that can also appear in the Smart Stack

**Videos:** Design widgets for the Smart Stack on Apple Watch · Go further with Complications in WidgetKit · Complications and widgets: Reloaded

## Web translation *(derived — not from Apple)*

Apple's HIG contains no web guidance, and this topic is one of the clearest cases where the mapping simply doesn't hold. A complication is a hardware- and OS-bound concept: it occupies a fixed, watch-case-specific pixel slot defined by Apple down to the half-point across five wrist sizes, it's triggered by a wrist-raise gesture, it renders through a tinted mode tied to the physical bezel and a wearer-selected watch face color, and it lives inside a system-curated carousel the wearer configures on their own device. No web technology puts a page's content onto a piece of hardware the browser doesn't control, at a size the browser doesn't choose, in a slot a website can't request. Say this plainly rather than forcing an analogy: complications are platform-bound, full stop.

**One narrow principle does transfer: a strict, system-imposed update budget changes what you design for.** Apple limits how many times per day a complication's timeline can be updated and how many timeline entries the system stores, which forces a design decision — choose the few moments when a refresh is actually worth spending — rather than treating updates as free. The web has a structurally similar constraint in the Periodic Background Sync API, where the browser (not the site) decides how often a registered background sync may run, based on engagement heuristics it doesn't expose to the page. The lesson carries over even though the mechanism doesn't: when a platform rations how often you're allowed to refresh, spend that budget on the update a person is actually waiting for, not on keeping every value technically current.

**A second narrow principle: "identify essential, dynamic content, not a launcher" is the same idea as the widget and Live Activity guidance, restated for a smaller surface.** Wherever a web product does get a genuinely glanceable, OS-adjacent surface — a PWA's icon badge, a Document Picture-in-Picture window, a browser-extension toolbar icon — the same discipline applies: show the one number or state someone actually needs, not a static brand mark standing in for it.

## Do / Don't

| Do | Don't |
|---|---|
| Display relevant, frequently changing data | Ship a static complication that never appears to update |
| Support as many complication families as your content allows | Limit yourself to one family when more would reach more watch faces |
| Deep link each complication to its own relevant screen | Route every complication to the same generic app screen |
| Design tinted-mode-safe alternatives for full-color images | Rely on color alone to convey information |
| Use line widths of 2 points or greater | Use hairline strokes that disappear at a glance or in motion |
| Choose update times that make the data meaningfully fresh | Waste your limited daily timeline updates on low-value moments |
| Keep sensitive data off the Always-On watch face | Display private information that bystanders could read |
