...
In iOS and iPadOS, your Live Activity appears throughout the system using these presentations. The system also uses them to create default appearances for other contexts — for example, the compact presentation appears in the Dynamic Island on iPhone and consists of two elements that the system combines into a single view for Apple Watch and in CarPlay.
...
**Minimal.** When multiple Live Activities are active, the system uses the minimal presentation to display two of them in the Dynamic Island. One appears attached to the Dynamic Island while the other appears detached. Depending on its content size, the detached minimal presentation appears circular or oval. As with the compact presentation, people tap the minimal presentation to open its app or touch and hold it to see the expanded presentation.

**Expanded.** When people touch and hold a Live Activity in compact or minimal presentation, the system displays the expanded presentation.

**Lock Screen.** The system uses the Lock Screen presentation to display a banner at the bottom of the Lock Screen. In this presentation, use a layout similar to the expanded presentation. When you alert people about Live Activity updates on devices that don't support the Dynamic Island, the Lock Screen presentation briefly appears as a banner that overlays the Home Screen or other apps.
...
Keep content compact and snug within a margin that's concentric to the outer edge of the Live Activity. When separating a block of content, place it in an inset container shape or use a thick line. Don't draw content all the way to the edge of the Dynamic Island.

> **Tip (Apple):** To align nonrounded content in the rounded corners of the Live Activity view, it may be helpful to blur the nonrounded content in your drawing tool. When the content is blurred, it may be easier to find the positioning that best aligns with the outer perimeter of the view.
...
**Use color to express the character and identity of your app.** Live Activities in the Dynamic Island use a black opaque background. Consider using bold colors for text and objects to convey the personality and brand of your app. Bold colors make your Live Activity recognizable at a glance, stand out from other Live Activities, and feel like a small, glanceable part of your app. Bold colors can also help reinforce the relationship between elements in the Live Activity itself.
...
In addition to extending and contracting transitions, Live Activities use system and custom animations with a maximum duration of two seconds. The system doesn't perform animations on Always-On displays with reduced luminance.
...
**Make sure tapping the Live Activity opens your app at the right location.** Take people directly to related details and actions — don't make them navigate to find relevant information.
...
**Use standard margins to align your design with notifications.** The standard layout margin for Live Activities on the Lock Screen is 14 points. While tighter margins may be appropriate for elements like graphics or buttons, avoid crowding the edges and creating a cluttered appearance.
...
**Consider creating a custom layout if your Live Activity would benefit from larger text or additional information.** Instead of using the default appearance in CarPlay, declare support for an `ActivityFamily.small` supplemental activity family.
...
| Compact or minimal | iPhone 17 Pro | 230 |
| Compact or minimal | iPhone Air | 250 |
| Compact or minimal | iPhone 17 | 230 |
| Compact or minimal | iPhone 16 Pro Max | 250 |
| Compact or minimal | iPhone 16 Pro | 230 |
| Compact or minimal | iPhone 16 Plus | 250 |
| Compact or minimal | iPhone 16 | 230 |
| Compact or minimal | iPhone 15 Pro Max | 250 |
| Compact or minimal | iPhone 15 Pro | 230 |
| Compact or minimal | iPhone 15 Plus | 250 |
| Compact or minimal | iPhone 15 | 230 |
| Compact or minimal | iPhone 14 Pro Max | 250 |
| Compact or minimal | iPhone 14 Pro | 230 |
| Expanded | iPhone 17 Pro Max | 408 |
| Expanded | iPhone 17 Pro | 371 |
| Expanded | iPhone Air | 408 |
| Expanded | iPhone 17 | 371 |
| Expanded | iPhone 16 Pro Max | 408 |
| Expanded | iPhone 16 Pro | 371 |
| Expanded | iPhone 16 Plus | 408 |
| Expanded | iPhone 16 | 371 |
| Expanded | iPhone 15 Pro Max | 408 |
| Expanded | iPhone 15 Pro | 371 |
| Expanded | iPhone 15 Plus | 408 |
| Expanded | iPhone 15 | 371 |
| Expanded | iPhone 14 Pro Max | 408 |
| Expanded | iPhone 14 Pro | 371 |
...
**Key APIs**
- `ActivityFamily.small` — declares the supplemental activity family for a custom CarPlay layout
- `ContainerRelativeShape` — applies the Live Activity's own corner radius to your content
- `padding(_:_:)` — sets Live Activity margins, including the 14-point Lock Screen margin
- `activitySystemActionForegroundColor(_:)` — adjusts the system-generated dismiss button color

**Videos:** Live Activities essentials · Turbocharge your app for CarPlay · What's new in widgets

## Web translation *(derived — not from Apple)*

Apple's HIG contains no web guidance. The mappings below apply the same principles to the web; they are inference, not Apple policy.

**No web surface plays the role of a Live Activity, and the gap is structural, not cosmetic.** A Live Activity is a system-owned, continuously updating view that persists across the Dynamic Island, Lock Screen, StandBy, a Mac's menu bar, a Watch's Smart Stack, and CarPlay — all without the source app running in the foreground, and all through one shared layout the OS composes into each context. The web has no equivalent because it has no OS-level chrome a page can claim: a browser tab that isn't the active tab doesn't render, and even background service workers can't paint pixels into a lock screen, a menu bar, or a dashboard the browser doesn't own. State this plainly rather than reaching for a strained analogy.

**The closest present-day approximation is the Document Picture-in-Picture API, and it only covers one of six locations.** A `documentPictureInPicture` window stays visible and can keep updating its content while someone works in other tabs or apps, which mirrors "track progress at a glance" for exactly as long as the browser stays open. It has no counterpart to Lock Screen, Dynamic Island, StandBy, menu bar, Smart Stack, or CarPlay presence, and it disappears the moment the browser closes — a Live Activity is designed to keep running independent of whether the source app is even foregrounded.

**Web Push and the Notifications API cover "alert people only for essential updates," but not "update in place."** Apple's rule that a Live Activity should update its existing view rather than spawn a new one has a real web analogue: a push notification created with a shared `tag` replaces the prior notification with the same tag instead of stacking a new one, so a person sees one evolving status rather than a pile of stale alerts. But a notification is transient and dismissible by design — it does not remain pinned, interactive, and re-openable the way a Live Activity's Lock Screen banner does, so the fidelity is partial at best.

**The Badging API is the nearest thing to the minimal presentation, and it is far thinner.** `navigator.setAppBadge()` lets an installed PWA show a number or dot on its icon, which is the same category of "recognizable at a glance without opening the app" as the minimal Dynamic Island presentation. It carries no custom layout, no color, and no live content beyond a count, so treat it as a fallback signal, not a substitute experience.

**"Avoid displaying sensitive information because Live Activities are prominently visible" transfers directly and matters more on the web.** Push notification banners are just as publicly visible as a Lock Screen, and unlike Live Activities they can appear on a locked or shared machine with no equivalent of Face ID gating the content underneath. Redact sensitive fields in the notification body and let a tap open the authenticated page, exactly as Apple recommends for the Live Activity view.

**"Alert people only for essential updates" is the load-bearing principle for web push, because the failure mode is worse on the web.** Apple notes that people who can't control a Live Activity's frequency turn it off in Settings; on the web, the equivalent failure is a person revoking notification permission entirely, which is harder to win back than disabling one Live Activity. Treat every push as a scarce resource, batch non-urgent updates, and never use push for what a badge or an in-app refresh could convey instead.

**"Don't use a Live Activity to display ads" and "focus on simple, direct actions" both map onto push notification content discipline.** A notification whose body is promotional, or whose actions require more than a single tap to resolve, produces the same trust erosion Apple is warning against — the mechanism differs, but the reason (a channel granted for one purpose gets used for another, and permission gets revoked) is identical.

## Do / Don't

| Do | Don't |
|---|---|
| Offer Live Activities for tasks with a clear beginning and end, under eight hours | Use a Live Activity as an ambient permanent display |
| Show only the most important information at a glance | Try to display everything in the compact or minimal presentation |
| Match your app's visual aesthetic in both light and dark appearances | Use the entire app icon as a logo mark |
| Redact or summarize sensitive content | Display sensitive information directly on the Lock Screen |
| Update only when new content is available | Refresh the display when the underlying status hasn't changed |
| Alert people only for essential, attention-worthy updates | Send push notifications alongside Live Activities for the same update |
| Offer at most one simple, direct interactive element | Build app-like layouts with multiple buttons and toggles |
| End the Live Activity immediately when the task ends | Leave a finished Live Activity running indefinitely |
