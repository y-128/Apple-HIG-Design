...
| Item | Value |
|---|---|
| Recommended maximum groups in a context menu | About 3 |
| Submenu nesting depth | 1 level maximum |
...
**Key APIs**
- `destructive` — marks a context menu item as destructive (red text)
- `UIContextMenuInteractionDelegate` — controls the animated preview as a context menu reveals

## Web translation *(derived — not from Apple)*

Apple's HIG contains no web guidance. The mappings below apply the same principles to the web; they are inference, not Apple policy.

**The web's native `contextmenu` event is a mouse-only concept, and that's the central mapping problem.** Apple lists three reveal gestures — touch/pinch-and-hold, Control-click, and trackpad secondary click — and only the latter two have a direct browser equivalent (the `contextmenu` event fires on right-click and on Control-click on most platforms). Long-press on touch does *not* reliably fire `contextmenu` in mobile browsers, and where it does, it's easy to conflate with text-selection or scroll gestures the browser also binds to long-press. A web context menu built only around the `contextmenu` event silently loses Apple's entire touch reveal path; supporting touch requires a custom long-press gesture handler layered on top, with real care taken not to fight the browser's own selection and scroll-start heuristics.

**Overriding the native context menu is a trust cost the web makes more visible than native platforms do.** On macOS or iOS, every context menu on screen is either genuinely system-provided or built with system-provided APIs that inherit standard look and behavior. On the web, calling `preventDefault()` on `contextmenu` to replace the browser's own menu with a custom one is common but always slightly adversarial — it overrides functionality (like "Inspect", "Save Image As", browser-level find) that the user, not the site, expects control over. Apple's rule that context-menu items must always also exist somewhere in the main interface is a good discipline to import here specifically because it hedges against a user whose browser, extension, or accessibility tool suppresses your custom menu — the action must still be reachable another way.

**"Hide unavailable items, don't dim them" → still the right default, and still means removing the DOM node or `display: none`, not `aria-disabled`.** This is a real behavioral difference from the main-menu guidance in the Menus page, which asks for a dimmed, present-but-inert item. A context menu's job, per Apple, is to be a short, always-relevant list — so the correct web implementation is to compute the applicable item set and render only those items, not to render everything and disable half of it.

**Destructive-item placement and coloring → applies directly, and CSS makes the coloring trivial; the confirmation step is the part to not skip.** Marking a destructive item last, in red text, is easy to copy. The part of Apple's guidance worth preserving with equal weight is the *behavioral* half — that choosing a destructive action should trigger a confirmation step (Apple's action sheet/popover) rather than executing immediately, because a context menu is reached through a gesture that's easy to fire by accident.

**No settled web equivalent for a "preview" that animates out of the selected content.** Apple's iOS/iPadOS content preview (a condensed rendering of the item that morphs into the context menu) has no standard browser primitive; reproducing it means custom animation work with the View Transitions API or manual transform/opacity choreography, and it's a large enough investment that most web context-menu implementations reasonably skip it and go straight to a plain list.

## Do / Don't

| Do | Don't |
|---|---|
| Show only items relevant to the current selection | Include advanced or rarely used items in a context menu |
| Keep the item list short and to about three groups | Build a long, heavily scrolled context menu |
| Make every context-menu action available elsewhere in the main interface | Hide an action only inside a context menu |
| Hide unavailable items entirely | Dim unavailable items the way a regular menu does |
| Place destructive items last, marked as destructive | Bury a destructive item among neutral actions with no visual warning |
| Limit submenus to one level | Nest submenus inside submenus |
| Show keyboard shortcuts in main menus only | Repeat keyboard shortcuts inside a context menu |
