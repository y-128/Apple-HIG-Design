...
> **Note (Apple):** The system automatically adds an overflow menu in macOS or iPadOS when items no longer fit. Don't add an overflow menu manually, and avoid layouts that cause toolbar items to overflow by default.
...
> *Image caption:* The standard toolbar in macOS Notes includes a More menu with extra commands, shown in both a standard and a compact toolbar variant.
...
> *Image caption:* Top toolbar buttons appear in the corners above content; bottom toolbar buttons appear along the bottom edge.
...
> *Image caption:* A scrolling toolbar button hidden at rest, and shown after the person scrolls up.
...
| Item | Value |
|---|---|
| Maximum recommended window title length | Under 15 characters |
| Maximum recommended toolbar groups | About 3 |
| Minimum items for a More/pull-down grouping to feel worthwhile | Not separately specified here; see Pull-down buttons |
...
**Key APIs**
- `ScrollEdgeEffectStyle` — distinguishes the toolbar area from the content area without a custom background
- `UIBarButtonItem.SystemItem.fixedSpace` — inserts fixed space between text-labeled toolbar buttons
- `prefersLargeTitles` — iOS large-title navigation behavior
- `topBarLeading`, `topBarTrailing`, `bottomBar` — watchOS toolbar button placements
- `primaryAction` — watchOS scrolling toolbar button

**Videos:** Get to know the new design system

## Web translation *(derived — not from Apple)*

Apple's HIG contains no web guidance. The mappings below apply the same principles to the web; they are inference, not Apple policy.

**A toolbar is a landmark, and the web has a role for exactly that.** Apple's toolbar is a persistent, horizontally organized region containing navigation, title, and actions — structurally, this is what the ARIA `toolbar` role (or the HTML5 `<header>`/`nav` landmark pattern, depending on content) exists to describe: a container of controls that screen reader users can jump to directly rather than tabbing through one control at a time. Wrapping a toolbar's buttons in `role="toolbar"` also changes arrow-key behavior — arrow keys move focus between items within the toolbar the way they do inside a native macOS or iPadOS toolbar, instead of leaving each button as a separate tab stop. Skipping the role turns a toolbar back into an undifferentiated row of buttons for anyone not using a mouse.

**Leading/center/trailing zones → this is flexbox's native model, which is why it maps unusually cleanly.** Apple's three-zone toolbar (leading edge fixed, center flexible and collapsible, trailing edge always visible) is close to a direct description of `justify-content: space-between` with a center region that truncates or overflows before the edges do. The one thing that doesn't come free: Apple's automatic system-managed overflow menu in macOS/iPadOS has no browser-native equivalent — CSS can hide overflowing items, but converting them into an actual reachable "More" menu is application logic you have to write yourself.

**The "aim for a maximum of three groups" and "keep text-labeled actions separated" rules → still about scan cost, so they still apply.** These are stated as pixel/spacing rules but the underlying claim is cognitive — too many visual groups, or text buttons that visually run together, make a toolbar harder to parse at a glance. Nothing about a browser changes that; if anything, responsive toolbars that reflow at narrow viewports make discipline about grouping *more* important, since a cluttered toolbar has fewer places to hide overflow gracefully than a native, system-managed one does.

**Only one primary/prominent action, on the trailing side → still true, and still under-followed on the web.** This is the same "spend your one accent color carefully" principle as in Buttons, applied to a toolbar-scale context. A web toolbar with three tinted buttons has the same problem Apple is describing: no clear focal point.

**"Don't title windows with your app name" → applies to browser tab titles too, but the constraint is different.** A browser `<title>` competes for space with a favicon and the truncation behavior of a crowded tab strip, so a page title that's just the app's brand name is dead weight there for the same reason it's dead weight in a native window title — it tells you nothing about *where you are*. Prefer "Inbox — Mail" style titles that put the location first.

**Hiding toolbars for a distraction-free mode → straightforward with `position: sticky`/`fixed` toggling, but the "reliable way to restore it" requirement is easy to drop.** Apple's caveat that hidden chrome must have a dependable way back is the part worth holding onto; web "focus mode" implementations that hide navigation entirely with no persistent affordance to bring it back are a common regression once the toolbar is gone.

## Do / Don't

| Do | Don't |
|---|---|
| Group toolbar items by function and frequency of use | Add more than about three groups to a single toolbar |
| Let the system manage overflow in macOS and iPadOS | Build a manual overflow menu on top of the system one |
| Use the `.prominent` style for exactly one trailing action like Done | Apply prominent styling to multiple toolbar actions |
| Keep title text under 15 characters | Title a window with the app's own name |
| Insert fixed space between adjacent text-labeled buttons | Place a text-labeled button directly next to a symbol button with no separation |
| Make every toolbar item also available in the macOS menu bar | Rely on a customizable or hideable toolbar as the only place a command exists |
| In visionOS, use the system-provided toolbar and avoid pull-down menus inside it | Build a vertical toolbar in visionOS |
