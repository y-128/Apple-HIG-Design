...
**Use an action sheet — not an alert — to offer choices related to an intentional action.** For example, when people cancel the Mail message they're editing, an action sheet provides three choices: delete the edits (or the entire draft), save the draft, or return to editing. Although an alert can also help people confirm or cancel an action that has destructive consequences, it doesn't provide additional choices related to the action.
...
When your app is running in the Shared Space, visionOS displays an alert in front of the app's window, slightly forward along the z-axis. If someone moves a window without dismissing its alert, the alert remains anchored to the window. If your app is running in a Full Space, the system displays the alert centered in the wearer's field of view.
...
**Alert → the native `<dialog>` element in modal mode.** Apple's alert is a modal view whose defining trait is that it blocks interaction with everything behind it until dismissed. `<dialog>` opened via `showModal()` gives this natively: it manages the top layer, creates a backdrop, and traps focus inside the dialog. Building a modal from a plain `<div>` with `position: fixed` requires you to reimplement all of that by hand — focus trapping, `Escape`-to-close, and `inert` on the rest of the page — and it is easy to get subtly wrong.

**"Use alerts sparingly" → the interruption cost is even higher on the web.** A native alert has system-level weight; a web modal competes with browser chrome, other tabs, and a user's mental model of "just a webpage." Reserve `<dialog>` for genuinely blocking situations — confirming a destructive, unrecoverable action — and prefer inline validation or a toast for anything merely informative.

**"Avoid using an alert merely to provide information" → this is where the web's toast pattern earns its keep.** Apple's own alternative for non-actionable information (a nonintrusive Mail indicator) has a close web equivalent in a toast or snackbar: an `aria-live` region that announces and self-dismisses without stealing focus or blocking input. A dialog demands a decision; a toast doesn't. If nothing about the message requires the user to choose between options, it belongs in a toast, not a dialog.

**Title and button-title discipline → applies directly, with one addition.** Apple's insistence on a specific, non-vague title ("Erase," not "OK") maps unchanged to the web — a button labeled "Confirm" is exactly the ambiguity Apple warns against. The web addition is that the dialog's accessible name (via `aria-labelledby` pointing at the title, or the dialog's own heading) must match the visible title, since screen reader users hear the accessible name before anything else.

**Default/Cancel placement and Escape-to-dismiss → native `<dialog>` gives you the second one for free.** `<dialog>` closes on `Escape` by default, matching Apple's Escape/Command-Period guidance. Trailing-side default button placement and destructive styling remain a design decision you implement with ordinary CSS and a `.destructive` class or equivalent — there's no platform default to lean on here, so be deliberate about it, matching the reading order convention (trailing for confirm, leading for cancel) your locale's users expect.

**Focus return on close → don't skip it.** When a native alert closes, focus returns to a sensible place automatically. `<dialog>` does return focus to the triggering element when closed via the API, but only if you invoked it correctly (store a reference to the trigger, or rely on the browser's default) — this is one of the more commonly-missed details in hand-rolled modals, and skipping it breaks the experience badly for keyboard and screen-reader users.

## Do / Don't

| Do | Don't |
|---|---|
| Use an alert for critical, actionable information | Show an alert for information with no action to take |
| Alert on uncommon, unrecoverable destructive actions | Alert on every common, undoable delete |
| Write a specific, complete title describing the situation | Use a vague title like "Error" |
| Use a specific button title like "Erase" or "Delete" | Default to "OK" for anything but purely informational alerts |
| Place the default button trailing (row) or top (stack) | Make a Cancel button the default button |
| Include a Cancel button alongside any destructive action | Skip Cancel when an action is destructive and unintended |
| Use the destructive style for accidental destructive actions | Apply destructive styling to a deliberately chosen action like Empty Trash |
