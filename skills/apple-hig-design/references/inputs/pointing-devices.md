...
iPadOS defines three content effects that bring focus to different types of interactive elements: **highlight**, **lift**, and **hover**.

- **Highlight** transforms the pointer into a translucent, rounded rectangle that acts as a background for a control and includes a gentle parallax. The subtle highlighting and movement bring focus to the control without distracting people from their task. By default, iPadOS applies the highlight effect to bar buttons, tab bars, segmented controls, and edit menus.
- **Lift** combines a subtle parallax with the appearance of elevation to make an element seem like it's floating above the screen. As the pointer fades out beneath the element, iPadOS creates the illusion of lift by scaling the element up while adding a shadow below it and a soft specular highlight on top of it. By default, iPadOS applies the lift effect to app icons and to buttons in Control Center.
...
> **Source limitation:** The source page embeds short looped videos demonstrating the pointer shape change, the highlight effect, the lift effect, and a custom hover effect. These are interactive video players in the PDF capture, not captioned images, so no caption text exists to reproduce here.
...
**When possible, support the system-provided content effects.** People quickly become accustomed to the content effects they see throughout the system and generally expect their experience to apply to every app they use. To provide a consistent user experience, align your interactions with the design intent of each effect:
...
| Horizontal I beam | Selection and insertion of text is possible in a horizontal layout, such as a TextEdit or Pages document. | `iBeam` |
...
**Hit region padding:** about 12 points around elements that include a bezel; about 24 points around the visible edges of elements without a bezel.
...
**Videos:** Design for the iPadOS pointer

## Web translation *(derived — not from Apple)*

Apple's HIG contains no web guidance. The mappings below apply the same principles to the web; they are inference, not Apple policy.

**Pointer detection and adaptive UI → `hover` and `pointer` media queries.** Apple's iPadOS pointer system exists because the OS can tell precisely when a pointing device is attached versus when someone is using a finger, and it adapts hit-target sizing, cursor shape, and interaction affordances accordingly. The web's closest equivalent is the CSS media features `(hover: hover)` / `(hover: none)` and `(pointer: fine)` / `(pointer: coarse)` — they let you detect whether the current input can hover and how precise it is, and adjust hit-target size or hover-only affordances accordingly. The gap: these are coarse binary signals, not the rich, continuously-updated "is a pointer currently over this element and how is it approaching" state Apple's pointer system has access to, and a touchscreen laptop with an attached mouse can report `hover: hover` even when the person is currently using their finger, so the query describes device *capability*, not current input *modality*, in a way Apple's system-level tracking does not conflate.

**Content effects (highlight, lift, hover) → CSS `:hover` plus transitions, with an honest ceiling.** The three content effects Apple defines are polished, physically-motivated (parallax, elevation, magnetism) responses baked into the OS compositor. `:hover` combined with `transform`, `box-shadow`, and `transition` can approximate the visual outcome — scale-up-with-shadow for lift, background-tint-with-parallax-adjacent-motion for highlight — but CSS has no pointer-magnetism equivalent: there is no way for a web element to visually "pull" the cursor itself, because the OS, not the page, owns cursor position. Any magnetism-like behavior on the web is target-size generosity (Fitts's-law-friendly large hit areas), not an attraction effect on the pointer.

**Custom cursor shapes → the CSS `cursor` property, with a much smaller vocabulary.** Apple's AppKit pointer table (arrow, crosshair, resize variants, I-beam, drag-copy, drag-link, operation-not-allowed) maps closely to CSS `cursor` keywords (`default`, `crosshair`, `ns-resize`/`ew-resize`, `text`, `copy`, `alias`, `no-drop`) — this is one of the closer 1:1 mappings in this document, since both systems are drawing from the same conceptual well of long-established desktop cursor semantics. `cursor: url(...)` allows fully custom cursor images for anything the keyword set doesn't cover, echoing Apple's advice to keep custom pointer shapes simple and self-explanatory.

**"Add padding around interactive elements for comfortable hit regions" → generous CSS hit-target sizing, especially given no OS-level magnetism.** Apple's 12pt/24pt padding guidance exists partly because pointer magnetism compensates for a tight target. The web has no magnetism to lean on, so hit-target generosity has to do more work unassisted — treat Apple's numbers as a floor, not a ceiling, and prefer padding the clickable area (via padding, not just visual size) over shrinking it to match a compact visual design.

**Pointer accessories and annotations → custom tooltips, with the caveat that mouse-only affordances fail touch and keyboard users.** Apple's pointer accessories communicate interaction hints attached to the cursor; a web `title` attribute or custom tooltip component is the nearest analogue, but any interaction that depends on `:hover` alone is invisible to touchscreen and keyboard-only users, so Apple's underlying principle — provide multiple ways to discover the same information — has to be honored more deliberately on the web than a single hover tooltip permits.

**visionOS attached-pointer-plus-gaze context switching → no web analogue.** The behavior where eye-gaze location determines which window a pointer acts on is unique to visionOS's spatial input model and has nothing corresponding to it in a 2D browser viewport.

## Do / Don't

| Do | Don't |
|---|---|
| Keep systemwide trackpad/mouse gestures available and unmodified | Redefine a systemwide gesture inside your app |
| Provide the same result whether a modifier-key interaction happens via touch or pointer | Make modifier-key behavior differ by input method |
| Use highlight for small, transparent-background elements | Apply the lift effect to elements it wasn't designed for |
| Use lift for small, opaque-background elements | Add scale and shadow to a cramped element with no room to grow |
| Use hover for large elements with custom scale/tint/shadow | Add gratuitous pointer or content effects with no functional purpose |
| Pad interactive elements for comfortable hit regions (~12pt bezeled, ~24pt unbezeled) | Leave hit regions so small that precision feels required |
| Keep custom pointer shapes simple and self-explanatory | Display instructional text via the pointer itself |
