...
Not supported in iOS, iPadOS, macOS, visionOS, or watchOS. The Siri Remote and compatible remotes are tvOS-only input devices.
...
Apple's HIG contains no web guidance, and this topic has no meaningful web analogue. The Siri Remote is a dedicated tvOS input device — a clickpad-plus-touch-surface with system-level gesture recognition (swipe, press, positional tap), a hardware Back button wired into the OS's app-hierarchy model, and a Play/Pause button tied into system media session state. A web page running in a browser, even a browser running on a TV or set-top box, has no standard way to receive Siri Remote input directly; any such integration happens at the OS/browser-shell level, not through a web API, and none of it is guaranteed to exist.

The one thing that generalizes is not an API but an interaction principle: distinguishing an intentional "press" (confirm/select) from an imprecise "tap/hover-like" gesture, and deliberately ignoring the imprecise one during states where an accidental trigger would be disruptive (Apple's example is live video playback). That same caution — don't let an ambiguous, low-intent gesture perform a real action during a sensitive state — is sound advice for any input device, including a web page reacting to touch or pointer events, but it is a general design principle, not something the Siri Remote uniquely enables on the web.

## Do / Don't

| Do | Don't |
|---|---|
| Use standard remote gestures for standard actions | Redefine standard remote behavior outside of gameplay |
| Move focus in the same direction as the gesture | Break the tvOS focus experience's directional consistency |
| Give clear feedback for what a gesture will do | Leave people guessing what a swipe or press does |
| Treat press as intentional, tap as more casual | Respond to every incidental tap, especially during live playback |
| Open the parent of the current screen on Back | Ignore the app hierarchy when handling Back outside gameplay |
| Use an in-game pause menu for Back during gameplay | Let repeated Back presses interrupt active gameplay |
| Respond to EPG "guide," "browse," and page buttons as expected | Repurpose EPG-browsing buttons for unrelated behavior |
| Change the channel on page up/down during content playback | Treat page up/down the same while playing content and browsing the EPG |
