...
> **Important (Apple):** To help preserve people's privacy, visionOS doesn't provide direct information about where people are looking before they tap. When you use system-provided components, visionOS automatically tells you when people tap the component.

visionOS also supports focus effects that help people navigate apps and the system using a connected input device like a keyboard or game controller. Focus effects are unrelated to the hover effect; see Focus and selection.
...
Not supported in iOS, iPadOS, macOS, tvOS, or watchOS. This entire page describes visionOS-only functionality.

## Specifications

| Guidance | Value |
|---|---|
| Minimum comfortable viewing distance for read/engaged content | At least 1 meter |
| Minimum margin around each interactive item | At least 16 points |
| Minimum distance between item centers (alternative to margin) | At least 60 points |
...
**Videos:** Design hover interactions for visionOS · Design for spatial input · Design considerations for vision and motion

## Web translation *(derived — not from Apple)*

Apple's HIG contains no web guidance, and this particular page describes gaze-tracking hardware — the eye cameras in Apple Vision Pro — that has no counterpart in any current web platform. Browsers do not have API access to gaze direction, and no headset today exposes raw eye-tracking data to web content even when it exposes it to native apps, both for the privacy reasons Apple itself cites in this document ("visionOS doesn't provide direct information about where people are looking before they tap") and because no web standard defines a gaze input event. Be honest about this: **there is no meaningful web analogue to gaze-driven hover, gaze-plus-pinch selection, custom hover-effect delay tuning, or anything else on this page that depends on knowing where a person's eyes are pointed.** A web developer reading this page should not expect to implement any of it; `:hover` in CSS is a pointer-proximity state, not a gaze state, and conflating the two would misrepresent what the technology does.

What does transfer is a narrower principle sitting underneath the specific mechanism: **generous, well-separated hit targets and adequate spacing between interactive elements make targeting easier for imprecise input, whatever that input is.** Apple's 16-point margin / 60-point center-to-center spacing exists because eyes drift in small involuntary saccades even while fixed on a target, so items need breathing room to stay individually targetable. The same underlying problem — imprecise pointing — recurs on the web with touch input (a fingertip is a blunt instrument compared to a mouse cursor) and with switch-access or head-tracking assistive input. The Web Content Accessibility Guidelines' target-size success criteria (WCAG 2.5.5 and 2.5.8, commonly cited as a 44×44 CSS-pixel minimum, with 24×24 as the AA-level floor) exist for the same underlying reason as Apple's 16pt/60pt numbers: sloppy input needs generous, well-separated targets, not precise ones. Treat the visionOS numbers as evidence for why generous spacing matters on any imprecise-input platform, not as values to port literally — a CSS pixel and a visionOS point are not the same unit, and the two guidance sets were derived for entirely different input mechanisms.

**"Avoid requiring rapid attention shifts across a large area or multiple depths" → the closest web equivalent is avoiding visual layouts that force rapid, wide eye movement, which is really a general readability principle, not something the web has special tooling for.** There's no depth axis on a 2D page, so the "multiple levels of depth" half of this guidance doesn't transfer at all; only the "keep related interactive content within a comfortable visual span" half has any web relevance, and it's closer to basic layout hygiene (keep a primary action and its context in the same viewport region) than to anything gaze-specific.

**Rounded shapes being easier to target with eyes → no evidence this transfers to pointer or touch input**, so this is not a general web design principle worth adopting on the strength of this page; it is specifically about how eyes behave at shape corners, not about mice or fingers.

## Do / Don't

| Do | Don't |
|---|---|
| Give people multiple ways to interact beyond gaze-and-pinch | Assume gaze is the only viable input for a task |
| Place content people will engage with at length at least 1 meter away | Place long-engagement content very close to the viewer |
| Give interactive items at least 16 points of margin, or 60 points between centers | Crowd interactive items so closely that eyes jump between them involuntarily |
| Prefer rounded shapes for items people target with their eyes | Rely on sharp-cornered shapes for gaze targets |
| Provide an overall containing shape for multi-element interactive components | Leave a multi-part component with no unified highlightable region |
| Keep at least one primary view stable during a custom hover effect | Change every part of an element simultaneously during a hover transition |
| Choose hover-effect delay based on how people are expected to interact | Use a long delay for a subtle, interaction-inviting effect like a slider knob |
